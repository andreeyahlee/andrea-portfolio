...
**"Play" nav item added, 2026-07-19 (Figma node `784:30857` for the nav row, `1399:72921` "Menu - Play" for the dropdown content):** link set is now **Work / Play / About / Contact**. "Play" is a second dropdown, built identically to "Work" (same `.nav__work-trigger`/`.nav__work-chevron`/`.nav__work-menu` classes reused — the `--work` naming is now generic/shared, not literal) with its own ids (`nav-play-item`, `nav-play-trigger`, `nav-play-menu`) so both dropdowns can be wired independently. Contents: "Side quests" → `works.html#other-works` (works.html's existing "In the Lab" section — code comments there already called these rows "Side Quests" before the nav even existed, so the id/label mismatch is pre-existing, not a bug) and "Event Stylings" → `works.html#event-styling` (works.html's existing Section 3). Both are same-page anchors on `works.html` itself and cross-page anchors (`works.html#...` / `../works.html#...`) everywhere else. The nav JS was refactored from single-dropdown (`workItem`/`workTrigger`) to a generic `navDropdowns` array so opening one dropdown closes the other — when adding a third dropdown, extend that array rather than writing new one-off handlers. Mobile menu gained a matching "Play" link (→ `#other-works`), same plain-link-no-submenu treatment as "Work" there.
